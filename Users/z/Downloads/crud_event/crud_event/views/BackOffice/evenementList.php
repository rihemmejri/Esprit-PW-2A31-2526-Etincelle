...
if (!empty($search)) {
    $allEvenements = $evenementController->listEvenements();
    $evenements = array_filter($allEvenements, function($e) use ($search) {
        return stripos($e->getTitre(), $search) !== false
            || stripos($e->getLieu(), $search) !== false;
    });
} else {
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .type-badge { display:inline-block; padding:5px 12px; border-radius:20px; font-size:0.75rem; font-weight:600; }
        ...
        .stats-modal-content { background:white; border-radius:24px; padding:36px; width:90%; max-width:620px; box-shadow:0 30px 80px rgba(0,0,0,0.3); animation: slideUpModal 0.4s cubic-bezier(0.34,1.56,0.64,1); }
...
<div class="container">
    ...
    <div class="stats-bar">
        <div class="stats">
            ...
            <div class="stat"><i class="fas fa-ticket-alt"></i><span>Places totales : <strong><?= $totalPlaces ?></strong></span></div>
        </div>
        <div class="search-box">
            <input type="number" id="searchId" placeholder="🔢 Rechercher par ID..." onkeyup="filterTable()" min="1">
            <input type="text" id="searchInput" placeholder="🔍 Titre, lieu..." onkeyup="filterTable()" value="<?= htmlspecialchars($search) ?>">
        ...
        <table id="evenementsTable">
            <thead>
                <tr>
                    ...
                    <th onclick="sortTable(7)"><i class="fas fa-circle"></i> Statut <span class="sort-icon"><i class="fas fa-sort"></i></span></th>
            ...
            <?php if (count($evenements) > 0): ?>
                ...
                <tr data-type="<?= $ev->getTypeEvenement() ?>"
                    data-statut="<?= $ev->getStatut() ?>"
                    data-id="<?= $ev->getIdEvenement() ?>">
                    ...
                    <td style="max-width:200px;white-space:normal;word-wrap:break-word">
                        <?= htmlspecialchars(substr($ev->getDescription(), 0, 80)) ?><?php if(strlen($ev->getDescription())>80): ?>...<?php endif; ?>
                    </td>
                    <td>
                        <?php $typeIcons=['SPORT'=>'fa-running','NUTRITION'=>'fa-apple-alt','WORKSHOP'=>'fa-chalkboard-teacher','AUTRE'=>'fa-calendar']; $icon=$typeIcons[$ev->getTypeEvenement()]??'fa-calendar'; ?>
                        <span class="type-badge type-<?= $ev->getTypeEvenement() ?>">
                            <i class="fas <?= $icon ?>"></i> <?= $ev->getTypeEvenement() ?>
                        </span>
                    </td>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="9" class="empty-message"><i class="fas fa-calendar-times"></i><p>Aucun événement trouvé</p></td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<div class="stats-modal" id="statsModal" onclick="closeStats(event)">
    <div class="stats-modal-content">
        <div class="stats-modal-header">
            <h2><i class="fas fa-chart-pie"></i> Statistiques par Type</h2>
        ...
        <div class="stats-donut-wrapper">
            <div class="donut-container">
                ...
                <div class="donut-center">
                    <span class="total-num"><?= count($evenements) ?></span>
...
<script>
    ...
    function filterTable() {
        ...
        const rows        = document.querySelectorAll('#evenementsTable tbody tr');
        ...
        rows.forEach(row => {
            ...
            row.style.display = (matchId && matchTexte && matchType) ? '' : 'none';
        });
    }
    ...
    // ===== STATISTIQUES DONUT =====
    const STATS_EV = [
        { label: 'Sport',     value: <?= count(array_filter($evenements, fn($e) => $e->getTypeEvenement() === 'SPORT')) ?>,     color: '#1565c0' },
        { label: 'Nutrition', value: <?= count(array_filter($evenements, fn($e) => $e->getTypeEvenement() === 'NUTRITION')) ?>, color: '#4CAF50' },
        { label: 'Workshop',  value: <?= count(array_filter($evenements, fn($e) => $e->getTypeEvenement() === 'WORKSHOP')) ?>,  color: '#e65100' },
        { label: 'Autre',     value: <?= count(array_filter($evenements, fn($e) => $e->getTypeEvenement() === 'AUTRE')) ?>,     color: '#6a1b9a' },
    ];
    const TOTAL_EV = <?= count($evenements) ?>;
    ...
    function drawDonut() {
        ...
        data.forEach((item, i) => {
            ...
            const pct  = item.value / (TOTAL_EV || 1);
            const dash = pct * circ;
            const gap  = circ - dash;
            ...
            leg.innerHTML += `
                <div class="legend-item">
                    ...
                    <div class="legend-info">
                        ...
                        <div class="legend-val">${item.value} événement(s) — ${pctStr}</div>
                </div>`;
        });
    }
    ...
    function exportPDF() {
        ...
        doc.text('Rapport des Evenements Nutritionnels', pageW/2, 13, {align:'center'});
        doc.setFontSize(9); doc.text('Genere le : ' + now, pageW-12, 13, {align:'right'});

        const statsData = [
            { label:'Total',         value:'<?= count($evenements) ?>', color:[76,175,80] },
            { label:'Actifs',        value:'<?= $totalActifs ?>',       color:[33,150,243] },
            { label:'Annules',       value:'<?= $totalCancelled ?>',    color:[244,67,54] },
            { label:'Places totales',value:'<?= $totalPlaces ?>',       color:[57,73,171] },
        ];
        let sx = 12;
        statsData.forEach(s => {
            doc.setFillColor(...s.color); doc.roundedRect(sx,27,60,16,3,3,'F');
            doc.setTextColor(255,255,255); doc.setFontSize(14); doc.setFont('helvetica','bold');
            doc.text(s.value, sx+30, 35, {align:'center'});
            doc.setFontSize(8); doc.setFont('helvetica','normal');
            doc.text(s.label, sx+30, 40, {align:'center'});
            sx += 65;
        });

        const headers   = ['ID', 'Titre', 'Type', 'Date', 'Lieu', 'Places', 'Statut'];
        const colWidths = [15, 65, 28, 25, 45, 18, 28];
        ...
        rows.forEach(row => {
            ...
            values.forEach((val,i) => {
                if (i===6) {
                    if (val==='ACTIF') doc.setTextColor(46,125,50);
                    else if (val==='CANCELLED') doc.setTextColor(198,40,40);
                    else doc.setTextColor(57,73,171);
        ...
        doc.text(rowCount+' evenement(s)', pageW-12, pageH-4, {align:'right'});
