...
// Prix max pour le slider
$prixValues = array_map(fn($e) => $e->getPrix(), $evenements);
$maxPrix    = count($prixValues) > 0 ? (int)ceil(max($prixValues)) : 100;
$maxPrix    = max($maxPrix, 50); // minimum 50 TND
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
.slider-val{font-size:14px;font-weight:700;color:#2196F3;min-width:80px;text-align:right;}
...
.slider-prix-display{text-align:center;margin-top:8px;font-size:11px;color:#757575;}
.slider-prix-display strong{color:#1a1a2e;font-size:12px;}
...
.prix-payant{background:#e3f2fd;color:#1565c0;}.prix-gratuit{background:#e8f5e9;color:#2e7d32;}
...
<div class="filter-panel">
  ...
  <div class="fp-row">
    ...
    <div class="fp-slider-wrap">
      <div class="fp-group-label">Filtrer par prix</div>
      <div class="fp-slider-block">
        <div class="slider-inner">
          <span class="slider-lbl">Jusqu'à</span>
          <input type="range" class="fp-range" id="sliderPrix"
                 min="0" max="<?= $maxPrix ?>" value="<?= $maxPrix ?>" step="5"
                 oninput="onPrixSlider(this)">
          <span class="slider-val" id="sliderPrixVal"><?= $maxPrix ?> TND</span>
        </div>
        <!-- Ticks prix fixes -->
        <div class="slider-ticks" id="sliderTicks"></div>
        <div class="slider-prix-display">Événements jusqu'à <strong id="sliderPrixDisplay"><?= $maxPrix ?> TND</strong></div>
      </div>
    </div>
  ...
  <table id="evenementsTable">
    ...
    <?php if (count($evenements) > 0): ?>
      ...
      <tr data-titre="<?= strtolower(htmlspecialchars($ev->getTitre())) ?>"
        ...
        <td>
          <?php if ($ev->isPayant()): ?>
            <span class="prix-badge prix-payant"><i class="fas fa-credit-card"></i> <?= number_format($ev->getPrix(), 2) ?> TND</span>
          <?php else: ?>
            <span class="prix-badge prix-gratuit"><i class="fas fa-gift"></i> Gratuit</span>
          <?php endif; ?>
        </td>
...
<div class="stats-modal-overlay" id="statsModal" onclick="if(event.target===this)closeStats()">
  <div class="stats-modal-box">
    ...
    <div class="smb-body">
      <svg id="chart3dEv" viewBox="0 0 560 240" width="100%" role="img">
        <defs>
...
<script>
const MAX_PRIX = <?= $maxPrix ?>;
let prixFilter = MAX_PRIX;
let typeFilter = null;

// ── Ticks prix fixes : Gratuit, puis par paliers de 10 TND ──
function buildTicks() {
    const c = document.getElementById('sliderTicks');
    c.innerHTML = '';
    const ticks = [0]; // 0 = Gratuit
    for (let p = 10; p <= MAX_PRIX; p += 10) ticks.push(p);
    ticks.forEach(val => {
        const t = document.createElement('div');
        t.className = 'tick' + (val === MAX_PRIX ? ' active' : '');
        t.dataset.v = val;
        const lbl = val === 0 ? 'Grat.' : val + ' TND';
        t.innerHTML = `<div class="tick-line"></div><div class="tick-lbl">${lbl}</div>`;
        t.addEventListener('click', () => {
            document.getElementById('sliderPrix').value = val;
            onPrixSlider(document.getElementById('sliderPrix'));
        });
        c.appendChild(t);
    });
}

function onPrixSlider(sl) {
    prixFilter = parseInt(sl.value);
    const pct = (prixFilter / MAX_PRIX) * 100;
    sl.style.background = `linear-gradient(to right,#2196F3 0%,#4CAF50 ${pct}%,#e0e0e0 ${pct}%)`;

    let label;
    if (prixFilter === 0) {
        label = 'Gratuit uniquement';
        document.getElementById('sliderPrixVal').textContent = 'Gratuit';
    } else {
        label = prixFilter + ' TND';
        document.getElementById('sliderPrixVal').textContent = prixFilter + ' TND';
    }
    document.getElementById('sliderPrixDisplay').textContent = label;

    document.querySelectorAll('.tick').forEach(t =>
        t.classList.toggle('active', parseInt(t.dataset.v) === prixFilter)
    );
    applyFilters();
}
...
function applyFilters() {
    ...
    document.querySelectorAll('#evenementsTable tbody tr').forEach(row => {
        ...
        // Filtre prix : si slider à 0 → gratuit seulement ; sinon prix <= prixFilter
        if (prixFilter === 0) {
            if (prix > 0) ok = false;
        } else {
            if (prix > prixFilter) ok = false;
        }
...
function resetFilters() {
    ...
    prixFilter = MAX_PRIX;
    const sl = document.getElementById('sliderPrix');
    sl.value = MAX_PRIX;
    sl.style.background = `linear-gradient(to right,#2196F3 0%,#4CAF50 100%,#e0e0e0 100%)`;
    document.getElementById('sliderPrixVal').textContent = MAX_PRIX + ' TND';
    document.getElementById('sliderPrixDisplay').textContent = MAX_PRIX + ' TND';
    document.querySelectorAll('.tick').forEach(t =>
        t.classList.toggle('active', parseInt(t.dataset.v) === MAX_PRIX)
    );
    applyFilters();
}
...
function sortTable(col) {
    const tbl = document.getElementById('evenementsTable');
...
function showPage(page) {
    ...
    const mkBtn = (lbl, p, isActive, disabled) => {
        ...
        b.onclick = () => showPage(p); pg.appendChild(b);
    };
...
function exportPDF() {
    ...
    document.querySelectorAll('#evenementsTable tbody tr').forEach(row=>{
        ...
        const vals=[cells[0]?.innerText.replace('#','').trim()||'',cells[1]?.innerText.trim()||'',cells[3]?.innerText.trim()||'',cells[4]?.innerText.trim()||'',cells[5]?.innerText.trim()||'',cells[6]?.innerText.trim()||'',cells[7]?.innerText.trim()||'',cells[8]?.innerText.trim()||''];
        let x=14; vals.forEach((v,i)=>{
            if(i===0){doc.setTextColor(33,150,243);doc.setFont('helvetica','bold');}
            else if(i===6){doc.setTextColor(v.includes('Gratuit')?46:21,v.includes('Gratuit')?125:101,v.includes('Gratuit')?50:192);doc.setFont('helvetica','bold');}
