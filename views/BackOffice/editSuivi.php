...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $suivi->setUserId($_POST['user_id'] ?? '');
    $suivi->setIdObjectif($_POST['id_objectif'] ?? null);
    $suivi->setDate($_POST['date'] ?? '');
    $suivi->setPoids($_POST['poids'] ?? '');
    $suivi->setCaloriesConsommees($_POST['calories_consommees'] ?? '');
    $suivi->setCaloriesObjectif($_POST['calories_objectif'] ?? '');
    $suivi->setEauBue($_POST['eau_bue'] ?? '');
    $suivi->setEauObjectif($_POST['eau_objectif'] ?? '');
    $suivi->setCaloriesRestant($suivi->getCaloriesObjectif() - $suivi->getCaloriesConsommees());

    $SuiviController->updateSuivi($suivi);
...
<body>
    <div class="form-container">
        <div class="form-header"><h1>Modifier le Suivi #<?= $suivi->getId() ?></h1></div>
        <form method="POST">
            <div class="form-row">
                <div class="form-group">
                    <label>User ID</label>
                    <input type="number" name="user_id" value="<?= $suivi->getUserId() ?>" required>
                </div>
                <div class="form-group">
                    ...
                    <select name="id_objectif" id="id_objectif">
                        ...
                        <?php foreach($objectifs as $o): ?>
                            <option value="<?= $o->getId() ?>" <?= ($suivi->getIdObjectif() == $o->getId()) ? 'selected' : '' ?> data-cal="<?= $o->getCaloriesObjectif() ?>" data-eau="<?= $o->getEauObjectif() ?>">Objectif #<?= $o->getId() ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <div class="form-group">
                <label>Date</label>
                <input type="date" name="date" value="<?= $suivi->getDate() ?>" readonly required>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label>Poids (kg)</label>
                    <input type="number" name="poids" step="0.1" value="<?= $suivi->getPoids() ?>" required>
                </div>
                <div class="form-group">
                    <label>Calories Consommées</label>
                    <input type="number" name="calories_consommees" value="<?= $suivi->getCaloriesConsommees() ?>" required>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label>Calories Objectif</label>
                    <input type="number" name="calories_objectif" id="calories_objectif" value="<?= $suivi->getCaloriesObjectif() ?>" readonly required>
                </div>
                <div class="form-group">
                    <label>Eau Bue (L)</label>
                    <input type="number" name="eau_bue" step="0.1" value="<?= $suivi->getEauBue() ?>" required>
                </div>
            </div>
            <div class="form-group">
                <label>Eau Objectif (L)</label>
                <input type="number" name="eau_objectif" id="eau_objectif" step="0.1" value="<?= $suivi->getEauObjectif() ?>" readonly required>
            </div>
            <div class="btn-group">
