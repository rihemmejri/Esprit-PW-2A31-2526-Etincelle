...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user_id && $poids_cible && $calories_objectif && $eau_objectif && $date_debut && $date_fin) {
        $objectif->setUserId($user_id);
        $objectif->setPoidsCible($poids_cible);
        $objectif->setCaloriesObjectif($calories_objectif);
        $objectif->setEauObjectif($eau_objectif);
        $objectif->setDateDebut($date_debut);
        $objectif->setDateFin($date_fin);
        
        $ObjectifController->updateObjectif($objectif);
        $_SESSION['success_message'] = 'Objectif modifié avec succès';
        header('Location: objectifList.php');
        exit();
    }
}
...
<body>
    <div class="form-container">
        <div class="form-header">
            <h1><i class="fas fa-edit"></i> Modifier l'Objectif</h1>
        </div>

        <form method="POST">
            <div class="form-group">
                <label for="user_id"><i class="fas fa-user"></i> User ID</label>
                <input type="number" id="user_id" name="user_id" required value="<?= htmlspecialchars($objectif->getUserId()) ?>" min="1">
            </div>

            <div class="form-row">
                <div class="form-group">
                    <label for="poids_cible"><i class="fas fa-weight"></i> Poids Cible (kg)</label>
                    <input type="number" id="poids_cible" name="poids_cible" required step="0.1" value="<?= htmlspecialchars($objectif->getPoidsCible()) ?>" min="0">
                </div>
                <div class="form-group">
                    <label for="calories_objectif"><i class="fas fa-fire"></i> Calories Objectif</label>
                    <input type="number" id="calories_objectif" name="calories_objectif" required value="<?= htmlspecialchars($objectif->getCaloriesObjectif()) ?>" min="0">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="eau_objectif" name="eau_objectif" required step="0.1" value="<?= htmlspecialchars($objectif->getEauObjectif()) ?>" min="0">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" id="date_debut" name="date_debut" required value="<?= htmlspecialchars($objectif->getDateDebut()) ?>">
                </div>
                <div class="form-group">
                    <label for="date_fin"><i class="fas fa-calendar"></i> Date Fin</label>
                    <input type="date" id="date_fin" name="date_fin" required value="<?= htmlspecialchars($objectif->getDateFin()) ?>">
                </div>
            </div>

            <div class="form-buttons">
                <button type="submit" class="btn-submit"><i class="fas fa-save"></i> Modifier</button>
